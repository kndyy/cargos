...
from cargos.core.models import (
    ExcelData,
    GenerationResult,
    AppConfig,
...
from cargos.core.constants import (
    METADATA_ROW_FECHA_SOLICITUD,
    METADATA_COL_FECHA_SOLICITUD,
    METADATA_ROW_TIENDA,
    METADATA_COL_TIENDA,
    METADATA_ROW_ADMINISTRADOR,
    METADATA_COL_ADMINISTRADOR,
    LOCATION_ROW,
    OCCUPATION_ROW,
    HEADER_ROW,
    DATA_START_ROW,
    IGNORE_COLUMN_INDEX,
    MAIN_DATA_END_COLUMN,
    UNIFORM_DATA_START_ROW,
...
class ExcelService:
    ...
    def load_excel_file(self, file_path: str) -> ExcelData:
        ...
        try:
            ...
            excel_data = ExcelData(file_path=file_path, worksheets=worksheets)

            self.logger.info(
                f"Excel file loaded successfully. {excel_data.successful_worksheets}/{excel_data.total_worksheets} worksheets parsed, {excel_data.total_people_parsed} people total."
            )
    ...
    def _parse_worksheet(
        ...
        try:
            ...
            try:
                if (
                    len(sheet_data) > METADATA_ROW_FECHA_SOLICITUD
                    and len(sheet_data.columns) > METADATA_COL_FECHA_SOLICITUD
                ):
                ...
                if (
                    len(sheet_data) > METADATA_ROW_TIENDA
                    and len(sheet_data.columns) > METADATA_COL_TIENDA
                ):
                    metadata.tienda = (
                        str(sheet_data.iloc[METADATA_ROW_TIENDA, METADATA_COL_TIENDA])
                        if pd.notna(
                            sheet_data.iloc[METADATA_ROW_TIENDA, METADATA_COL_TIENDA]
                        )
                        else ""
                    )

                if (
                    len(sheet_data) > METADATA_ROW_ADMINISTRADOR
                    and len(sheet_data.columns) > METADATA_COL_ADMINISTRADOR
                ):
                    metadata.administrador = (
                        str(
                            sheet_data.iloc[
                ...
                self.logger.warning(
                    f"Metadata extraction error in sheet '{sheet_name}': {str(meta_error)}"
                )
            ...
            try:
                if len(sheet_data) > DATA_START_ROW:  # Ensure we have data rows
                    ...
                    headers = sheet_data.iloc[
                        HEADER_ROW, IGNORE_COLUMN_INDEX + 1 : MAIN_DATA_END_COLUMN + 1
                    ]

                    # Read main data: from DATA_START_ROW onwards, columns B through I
                    main_data_rows = sheet_data.iloc[
                        DATA_START_ROW:,
                        IGNORE_COLUMN_INDEX + 1 : MAIN_DATA_END_COLUMN + 1,
                    ]
                    ...
                    if len(main_data_rows) > 0:
                        ...
                        if "DNI" in main_data_rows.columns or any(
                            "dni" in str(col).lower() for col in main_data_rows.columns
                        ):
                            ...
                            if dni_col is not None:
                                ...
                                if missing_dni_count > 0:
                                    result.errors.append(
                                        f"{missing_dni_count} rows with data are missing DNI"
                                    )
                        else:
                            result.warnings.append(
                                "No DNI column found - cannot validate DNI completeness"
                            )
                        ...
                        if len(sheet_data) > UNIFORM_DATA_START_ROW:
                            ...
                            actual_end_col = min(
                                UNIFORM_DATA_END_COLUMN + 1, len(sheet_data.columns)
                            )
                            uniform_data_rows = sheet_data.iloc[
                                UNIFORM_DATA_START_ROW:,
                                UNIFORM_DATA_START_COLUMN:actual_end_col,
                            ]
                            ...
                            for i in range(actual_col_count):
                                ...
                                if col_idx in UNIFORM_COLUMN_MAPPING:
                                    uniform_column_names.append(
                                        UNIFORM_COLUMN_MAPPING[col_idx]
                                    )
                                else:
                            ...
                            combined_data = pd.concat(
                                [main_data_rows, uniform_data_rows], axis=1
                            )
                            ...
                            main_data_rows = combined_data.iloc[
                                :, : len(main_data_rows.columns)
                            ]
                            uniform_data = combined_data.iloc[
                                :, len(main_data_rows.columns) :
                            ]
                        ...
                        result.people_parsed = len(
                            main_data_rows
                        )  # After cleaning empty rows
                        result.total_lines = len(sheet_data)  # Total lines in sheet

                        self.logger.info(
                            f"Sheet '{sheet_name}': {result.people_parsed} people parsed from {result.total_lines} total lines"
                        )
                        if uniform_data is not None:
                            self.logger.info(
                                f"Sheet '{sheet_name}': {len(uniform_data)} uniform data rows extracted"
                            )
                    else:
                        result.warnings.append("No data rows found after headers")
                else:
                    result.warnings.append(
                        f"Sheet has insufficient rows (less than {DATA_START_ROW + 1})"
                    )

            except Exception as data_error:
                result.errors.append(f"Error extracting data: {str(data_error)}")
                self.logger.error(
                    f"Data extraction error in sheet '{sheet_name}': {str(data_error)}"
                )
            ...
            self.logger.exception(
                f"Critical error parsing worksheet '{sheet_name}': {str(e)}"
            )
    ...
    def validate_excel_data(self, excel_data: ExcelData) -> ExcelValidationResult:
        ...
        try:
            if not excel_data.is_loaded:
                ...
                result.message = (
                    "Excel file could not be loaded or has no valid worksheets"
                )
                return result
            ...
            for worksheet in excel_data.worksheets:
                if worksheet.errors:
                    result.errors.extend(
                        [
                            f"Sheet '{worksheet.metadata.sheet_name}': {error}"
                            for error in worksheet.errors
                        ]
                    )
                ...
                if worksheet.warnings:
                    result.warnings.extend(
                        [
                            f"Sheet '{worksheet.metadata.sheet_name}': {warning}"
            ...
            if excel_data.successful_worksheets == 0:
                ...
                result.message = (
                    f"All {excel_data.total_worksheets} worksheets failed to parse"
                )
                return result
            ...
            if occupation_mapping_issues:
                result.warnings.append(
                    f"Occupation mapping issues found: {', '.join(sorted(occupation_mapping_issues))}"
                )
                result.warnings.append(
                    "Consider adding these occupations to the synonyms list in Configuration tab"
                )
...
class FileGenerationService:
    ...
    def _validate_generation_inputs(
        ...
        try:
            ...
            result.message = (
                "docxtpl not available; please install python-docx-template"
            )
    ...
    def _create_default_options(self, excel_data: ExcelData) -> GenerationOptions:
        ...
        locales = [
            w.metadata.tienda for w in excel_data.worksheets if w.metadata.tienda
        ]
    ...
    def _group_data_by_locale(
        self, excel_data: ExcelData, options: GenerationOptions
        ...
        for ws in excel_data.worksheets:
            ...
            if not tienda or tienda.strip() == "":
                self.logger.warning(
                    f"Sheet '{ws.metadata.sheet_name}' skipped: Missing tienda"
                )
                continue
    ...
    def _build_person_contexts(
        self, row: pd.Series, ws, options: GenerationOptions
    ) -> Dict[str, Any]:
        ...
        if options.autorizacion_enabled:
            autorizacion_ctx = self._build_autorizacion_context(
                row, ws.metadata, uniform_row
            )
            if autorizacion_ctx:
                person_contexts["AUTORIZACION"] = autorizacion_ctx
                self.logger.debug(
                    f"Built AUTORIZACION context for {autorizacion_ctx.get('nombre', 'unknown')}"
                )
            else:
        ...
        if options.cargo_enabled:
            ...
            if cargo_ctx:
                ...
                self.logger.debug(
                    f"Built CARGO context for {cargo_ctx.get('nombre', 'unknown')} with {len(cargo_ctx.get('prendas', []))} prendas"
                )
            else:
    ...
    def _generate_documents(
        ...
        for tienda, people in tienda_to_rows.items():
            ...
            if options.combine_per_local:
                files_generated += self._create_combined_documents(
                    template_docs, tienda_folder, tienda
                )
    ...
    def _generate_single_document(
        ...
        try:
            if template_type == "AUTORIZACION":
                docx_path = (
                    person_folder / f"AUTORIZACION_{self._file_stub(context)}.docx"
                )
                self.logger.info(f"Generating AUTORIZACION document: {docx_path}")
                self._render_autorizacion_doc(
                    config.autorizacion_template_path, context, docx_path
                )
                return docx_path
    ...
    def _create_combined_documents(
        self, template_docs: Dict[str, List[Path]], tienda_folder: Path, tienda: str
    ) -> int:
        ...
        for template_type, docs in template_docs.items():
            if docs:
                combined_path = (
                    tienda_folder
                    / f"{template_type}_COMBINED_{self._sanitize_name(tienda)}.docx"
                )
    ...
    def _has_valid_uniform_data(self, ws) -> bool:
        ...
        return ws.uniform_data is not None and hasattr(ws, "data") and len(ws.data) > 0

    def _calculate_juegos(
        self,
        row: pd.Series,
        metadata: WorksheetMetadata,
        uniform_row: Optional[pd.Series] = None,
    ) -> int:
        ...
        try:
            ...
            if not cargo:
                self.logger.warning(
                    f"No cargo found for person {person_name}, cannot calculate juegos"
                )
                return 0
            ...
            person_name = self._extract_name(row)
            self.logger.debug(
                f"Calculating juegos for {person_name}: cargo='{cargo}' -> normalized='{normalized_cargo}'"
            )

            # Get primary prenda for this occupation
            primary_prenda_cfg = self.unified_service.get_primary_prenda(
                normalized_cargo
            )
            if not primary_prenda_cfg:
                self.logger.warning(
                    f"No primary prenda configured for occupation: {normalized_cargo} (person: {person_name})"
                )
                return 0

            primary_prenda_type = primary_prenda_cfg.prenda_type.upper().strip()
            self.logger.debug(
                f"Primary prenda for {normalized_cargo}: {primary_prenda_type}"
            )
            ...
            if not prendas:
                self.logger.warning(
                    f"No prendas found for person {person_name}, cannot calculate juegos"
                )
                return 0

            self.logger.debug(
                f"Found {len(prendas)} prendas for {person_name}: {[p.get('prenda_type', '') + ' (qty=' + str(p.get('qty', 0)) + ')' for p in prendas]}"
            )
            ...
            if primary_prenda_found:
                ...
                if qty > 0:
                    juegos = int(qty)
                    self.logger.info(
                        f"Juegos calculated from primary prenda {primary_prenda_type} for {person_name}: {juegos}"
                    )
                    return juegos
                else:
                    self.logger.debug(
                        f"Primary prenda {primary_prenda_type} found but qty=0 for {person_name}, calculating mode of other prendas"
                    )
            else:
                self.logger.debug(
                    f"Primary prenda {primary_prenda_type} not found in prendas list for {person_name}, calculating mode of other prendas"
                )
            ...
            if not other_quantities:
                self.logger.warning(
                    f"No other prendas with qty > 0 found for {person_name}, returning 0 for juegos"
                )
                return 0
            ...
            try:
                ...
                modes = [
                    qty for qty, count in quantity_counts.items() if count == max_count
                ]
                # If tie, return the highest value
                juegos = int(max(modes)) if modes else 0

            self.logger.info(
                f"Juegos calculated from mode of other prendas for {person_name}: {juegos} (from quantities: {other_quantities})"
            )
            return juegos

        except Exception as e:
            person_name = (
                self._extract_name(row) if hasattr(self, "_extract_name") else "unknown"
            )
            self.logger.error(
                f"Failed to calculate juegos for person {person_name}: {e}",
                exc_info=True,
            )
            return 0

    def _build_autorizacion_context(
        self,
        row: pd.Series,
        metadata: WorksheetMetadata,
        uniform_row: Optional[pd.Series] = None,
        ...
        try:
            ...
            if not person_data["identificacion"]:
                self.logger.warning(
                    f"No DNI found for person: {person_data['nombre']}. Available columns: {list(row.index)}"
                )
            if not person_data["nombre"]:
                self.logger.warning(
                    f"No nombre found. Available columns: {list(row.index)}"
                )
            if not person_data["cargo"]:
                self.logger.warning(
                    f"No cargo found. Available columns: {list(row.index)}"
                )
            if not fecha_template:
                self.logger.warning(
                    f"No date found in metadata: {metadata.fecha_solicitud}"
                )
            ...
            self.logger.info(
                f"AUTORIZACION context for {person_data['nombre']}: juegos={juegos_value} (type: {type(juegos_value).__name__}), cargo={person_data['cargo']}, local={metadata.tienda}"
            )
    ...
    def _render_autorizacion_doc(
        self, template_path: str, context: Dict[str, Any], output_docx: Path
    ) -> None:
        """Render AUTORIZACION document."""
        self._render_document(template_path, context, output_docx)

    def _render_cargo_doc(
        self, template_path: str, context: Dict[str, Any], output_docx: Path
    ) -> None:
    ...
    def _render_document(
        ...
        self.logger.debug(
            f"Rendering document {output_docx.name} with context keys: {list(context.keys())}"
        )
        if "juegos" in context:
            self.logger.info(
                f"Context includes 'juegos' = {context['juegos']} (type: {type(context['juegos'])})"
            )
        else:
            self.logger.warning(
                f"Context does NOT include 'juegos'! Available keys: {list(context.keys())})"
            )
    ...
    def _create_combined_docx(
        self, individual_docs: List[Path], output_path: Path
    ) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Created combined document with preserved formatting: {output_path}"
            )
            ...
            self.logger.error(
                f"Failed to create combined document with docxcompose: {e}"
            )
            # Fallback to manual method
            self._create_fallback_combined_docx(individual_docs, output_path)

    def _create_fallback_combined_docx(
        self, individual_docs: List[Path], output_path: Path
    ) -> None:
        ...
        try:
            ...
            fallback_doc.add_paragraph(
                f"This document combines {len(individual_docs)} individual authorization documents."
            )
            ...
            fallback_doc.add_paragraph("")
            fallback_doc.add_paragraph(
                "Note: Please check individual documents for complete content."
            )
        ...
        try:
            ...
            if normalized_cargo != cargo.upper():
                ...
                if occupation_obj and occupation_obj.prendas:
                    ...
                    sample_price = f" (sample: {first_prenda.display_name} S/{first_prenda.price_sml_lima_ica})"
                ...
                if "(HOMBRE)" in normalized_cargo or "(MUJER)" in normalized_cargo:
                    gender = (
                        "👨 MALE" if "(HOMBRE)" in normalized_cargo else "👩 FEMALE"
                    )
                    self.logger.info(
                        f"🔔 GENDER SELECTION: '{cargo}' → '{normalized_cargo}' [{gender}]{sample_price}"
                    )
                else:
                    self.logger.warning(
                        f"⚠️  OCCUPATION MAPPING: '{cargo}' → '{normalized_cargo}'{sample_price} - consider adding '{cargo}' to synonyms"
                    )
            else:
                ...
                if occupation_obj and occupation_obj.prendas:
                    first_prenda = occupation_obj.prendas[0]
                    self.logger.info(
                        f"Occupation: '{normalized_cargo}' (sample: {first_prenda.display_name} S/{first_prenda.price_sml_lima_ica})"
                    )
            ...
            person_name = self._extract_name(row)
            if (
                normalized_cargo in ["PACKER", "MOTORIZADO"]
                or "PACKER" in cargo.upper()
                or "MOTORIZADO" in cargo.upper()
            ):
                ...
                if uniform_row is not None:
                    self.logger.info(
                        f"  Uniform row columns: {list(uniform_row.index)}"
                    )
            ...
            self.logger.info(
                f"Calculated monto for {person_name}: "
    ...
    def _apply_business_rules(
        self, prendas: List[Dict[str, Any]], row: pd.Series
    ) -> List[Dict[str, Any]]:
        ...
        is_male_admin = any(
            ...
            "ADMIN" in cargo_upper
            and "(M)" not in cargo_upper
            and "MUJER" not in cargo_upper
            and "(F)" not in cargo_upper
        )

        # Rule 1: Male admin gets 2 corbatas
        if is_male_admin:
            has_corbata = any(
                p.get("prenda_type", "").upper() == "CORBATA" for p in prendas
            )
            if not has_corbata:
                # Add corbata
                prendas.append(
                ...
                self.logger.info(
                    f"Business rule: Added 2 CORBATA for male admin '{cargo}'"
                )
            else:
                # Force quantity to 2
                for p in prendas:
                    if p.get("prenda_type", "").upper() == "CORBATA":
                        if p.get("qty", 0) != 2:
                            self.logger.info(
                                f"Business rule: Adjusted CORBATA qty from {p.get('qty')} to 2 for '{cargo}'"
                            )
                            p["qty"] = 2
        ...
        for p in prendas:
            if p.get("prenda_type", "").upper() == "SACO" and p.get("qty", 0) > 1:
                self.logger.info(
                    f"Business rule: Capped SACO qty from {p.get('qty')} to 1 for '{cargo}'"
                )
                p["qty"] = 1
    ...
    def _resolve_gendered_occupation(
        self, cargo: str, detected_gender: Optional[str]
    ) -> str:
        ...
        for pattern, (male_occ, female_occ) in gendered_patterns.items():
            if pattern in cargo_upper:
                if detected_gender == "MUJER":
                    self.logger.info(
                        f"Resolved gendered occupation: {cargo} -> {female_occ}"
                    )
                    return female_occ
                elif detected_gender == "HOMBRE":
                    self.logger.info(
                        f"Resolved gendered occupation: {cargo} -> {male_occ}"
                    )
                    return male_occ
                else:
                    ...
                    if self.gender_prompt_callback:
                        person_name = (
                            ...
                            if hasattr(self, "_current_person_name")
                            else "Unknown"
                        )
                        ...
                        if cache_key in self._gender_selection_cache:
                            ...
                            self.logger.info(
                                f"Using cached gender selection for {person_name}: {selected}"
                            )
                            return female_occ if selected == "MUJER" else male_occ

                        selected = self.gender_prompt_callback(
                            person_name, cargo, male_occ, female_occ
                        )
                        ...
                        if selected == "MUJER":
                            self.logger.info(
                                f"User selected MUJER for {cargo} -> {female_occ}"
                            )
                            return female_occ
                        else:
                            self.logger.info(
                                f"User selected HOMBRE for {cargo} -> {male_occ}"
                            )
                            return male_occ
                    else:
                        # No callback - log warning and default to male (fallback)
                        self.logger.warning(
                            f"Gender undetermined for '{cargo}' - no prompt callback, defaulting to male"
                        )
                        return male_occ
    ...
    def _sanitize_name(self, s: str) -> str:
        return (
            "".join(ch for ch in s if ch.isalnum() or ch in ("_", "-", " "))
            .strip()
            .replace(" ", "_")
        )
