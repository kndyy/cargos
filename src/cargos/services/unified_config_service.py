...
class UnifiedConfigService:
            ...
            for prenda_data in occ_data.get("prendas", []):
                try:
                    ...
                    if not garment_type:
                        ...
                        if any(
                            lower in prenda_type_upper
                            for lower in ["PANTALON", "PANTALÓN", "PANTS"]
                        ):
                            garment_type = "LOWER"
                        else:
                            garment_type = "UPPER"
    ...
    def calculate_total_price(
        ...
        self.logger.info(
            f"Calculating price for {cargo} -> {occupation.name} in {loc_str}"
        )
        ...
        for prenda in prendas:
            ...
            price = self.price_loader.get_price(
                occupation.name, prenda_type, size, loc_str
            )

            # Fallback to config.json price if Excel returns 0
            if price == 0.0:
                price = self.unified_config.get_price(
                    prenda_type, size, occupation.name, loc_str
                )
            ...
            if price > 0:
                self.logger.info(
                    f"  {prenda_string}: {prenda_type} TALLA {size} x{qty} = {price} x {qty} = {subtotal}"
                )
            else:
                self.logger.warning(
                    f"  ⚠️ NO PRICE FOUND for {prenda_type} (size {size}) in {occupation.name} at {loc_str}"
                )
    ...
    def _resolve_price(
        self, prenda_cfg: OccupationPrenda, size: str, local: str
    ) -> float:
    ...
    def add_prenda_to_occupation(
        self, occupation_name: str, prenda: OccupationPrenda
    ) -> bool:
        ...
        if any(p.prenda_type == prenda.prenda_type for p in occupation.prendas):
            self.logger.warning(
                f"Prenda {prenda.prenda_type} already exists in {occupation_name}"
            )
            return False
    ...
    def update_prenda_in_occupation(
        self, occupation_name: str, prenda: OccupationPrenda
    ) -> bool:
        ...
        self.logger.warning(
            f"Prenda {prenda.prenda_type} not found in {occupation_name}"
        )
        return False

    def delete_prenda_from_occupation(
        self, occupation_name: str, prenda_type: str
    ) -> bool:
        ...
        occupation.prendas = [
            p for p in occupation.prendas if p.prenda_type != prenda_type
        ]
        return True
    ...
    def remove_synonym_from_occupation(
        self, occupation_name: str, synonym: str
    ) -> bool:
    ...
    def get_prenda_from_occupation(
        self, occupation_name: str, prenda_type: str
    ) -> Optional[OccupationPrenda]:
    ...
    def update_prenda_pricing(
        ...
        if not prenda:
            ...
            if not occupation:
                self.logger.warning(
                    f"Occupation {occupation_name} not found for pricing update"
                )
                return False
        ...
        if hasattr(prenda, price_attr):
            ...
            self.logger.debug(
                f"Updated {occupation_name}/{prenda_type} {price_attr} = {price}"
            )
            return True
        else:
    ...
    def validate_occupation(self, occupation: Occupation) -> List[str]:
        ...
        for i, prenda in enumerate(occupation.prendas):
            ...
            if prenda_errors:
                errors.extend(
                    [
                        f"Prenda {i + 1} ({prenda.prenda_type}): {err}"
                        for err in prenda_errors
                    ]
                )
    ...
    def validate_prenda(self, prenda: OccupationPrenda) -> List[str]:
        ...
        price_fields = ["price_sml_tarapoto", "price_xl_tarapoto", "price_xxl_tarapoto"]
