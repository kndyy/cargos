...
from cargos.core.constants import (
    GENERATION_DIALOG_WIDTH,
    GENERATION_DIALOG_HEIGHT,
    GENERATION_DIALOG_CANVAS_HEIGHT,
    TREE_COLUMN_WIDTH_PEOPLE,
    TREE_COLUMN_WIDTH_STATUS,
    TREE_COLUMN_WIDTH_UNIFORM,
    TREE_COLUMN_WIDTH_DATA,
)
...
class CompactFileSelectionFrame:
    ...
    def __init__(self, parent, config: AppConfig):
        ...
        self.autorizacion_template_var = tk.StringVar(
            value=config.autorizacion_template_path
        )
    ...
    def _create_widgets(self):
        ...
        ttk.Label(self.frame, text="Formato Uniforme:").grid(
            row=0, column=0, sticky="w", padx=(0, 5)
        )
        self.excel_path_label = ttk.Label(
            self.frame, text="(no file)", width=30, anchor="w", foreground="gray"
        )
        self.excel_path_label.grid(row=0, column=1, sticky="w", padx=(0, 5))
        ttk.Button(self.frame, text="Browse", command=self._browse_excel_file).grid(
            row=0, column=2
        )
        self.load_button = ttk.Button(
            self.frame, text="Load", command=self._load_excel, state="disabled"
        )
        self.load_button.grid(row=0, column=3, padx=(5, 0))
        self.reload_button = ttk.Button(
            self.frame, text="Reload", command=self._reload_excel, state="disabled"
        )
        self.reload_button.grid(row=0, column=4, padx=(2, 0))

        # CARGO file (with toggle)
        self.cargo_checkbox = ttk.Checkbutton(
            self.frame,
            text="CARGO:",
            variable=self.cargo_enabled_var,
            command=self._on_cargo_toggle,
        )
        self.cargo_checkbox.grid(row=1, column=0, sticky="w", padx=(0, 5), pady=(3, 0))
        self.cargo_path_label = ttk.Label(
            self.frame,
            text=Path(self.cargo_template_var.get()).name
            if self.cargo_template_var.get()
            else "(no file)",
            width=30,
            anchor="w",
            foreground="gray",
        ...
        self.cargo_path_label.grid(
            row=1, column=1, sticky="w", padx=(0, 5), pady=(3, 0)
        )
        self.cargo_browse_btn = ttk.Button(
            self.frame, text="Browse", command=self._browse_cargo_template
        )
        self.cargo_browse_btn.grid(row=1, column=2, pady=(3, 0))

        # AUTORIZACION file (with toggle)
        self.autorizacion_checkbox = ttk.Checkbutton(
            self.frame,
            text="AUTORIZACION:",
            variable=self.autorizacion_enabled_var,
            command=self._on_autorizacion_toggle,
        )
        self.autorizacion_checkbox.grid(
            row=2, column=0, sticky="w", padx=(0, 5), pady=(3, 0)
        )
        self.autorizacion_path_label = ttk.Label(
            self.frame,
            text=Path(self.autorizacion_template_var.get()).name
            if self.autorizacion_template_var.get()
            else "(no file)",
            width=30,
            anchor="w",
            foreground="gray",
        ...
        self.autorizacion_path_label.grid(
            row=2, column=1, sticky="w", padx=(0, 5), pady=(3, 0)
        )
        self.autorizacion_browse_btn = ttk.Button(
            self.frame, text="Browse", command=self._browse_autorizacion_template
        )
        self.autorizacion_browse_btn.grid(row=2, column=2, pady=(3, 0))

        # Destination folder (compact)
        ttk.Label(self.frame, text="Destination:", font=("TkDefaultFont", 8)).grid(
            row=3, column=0, sticky="w", padx=(0, 5), pady=(3, 0)
        )
        self.destination_path_label = ttk.Label(
            self.frame,
            text=Path(self.destination_path_var.get()).name
            if self.destination_path_var.get()
            else "(no folder)",
            width=30,
            anchor="w",
            foreground="gray",
        ...
        self.destination_path_label.grid(
            row=3, column=1, sticky="w", padx=(0, 5), pady=(3, 0)
        )
        ttk.Button(
            self.frame, text="Browse", command=self._browse_destination_folder
        ).grid(row=3, column=2, pady=(3, 0))
    ...
    def _browse_excel_file(self):
        ...
        if file_path:
            ...
            self.excel_path_label.configure(
                text=Path(file_path).name, foreground="black"
            )
    ...
    def _on_autorizacion_toggle(self):
        ...
        self.autorizacion_browse_btn.configure(
            state="normal" if enabled else "disabled"
        )
        self.autorizacion_path_label.configure(
            foreground="black" if enabled else "gray"
        )
        if self.on_template_toggles_changed:
            self.on_template_toggles_changed()
    ...
    def _browse_cargo_template(self):
        ...
        if file_path:
            ...
            self.cargo_path_label.configure(
                text=Path(file_path).name, foreground="black"
            )
    ...
    def _browse_destination_folder(self):
        ...
        if folder_path:
            ...
            self.destination_path_label.configure(
                text=Path(folder_path).name, foreground="black"
            )
...
class WorksheetSummaryFrame:
    ...
    def _create_widgets(self):
        ...
        self.tree = ttk.Treeview(
            self.frame, columns=columns, show="headings", height=DEFAULT_TREE_HEIGHT
        )
        ...
        for worksheet in excel_data.worksheets:
            if worksheet.data is None:
                ...
                if has_critical_errors:
                    ...
                    has_occupation_issues = self._has_occupation_mapping_issues(
                        worksheet
                    )
    ...
    def _has_critical_errors(self, worksheet):
        ...
        if (
            not worksheet.metadata.tienda
            or str(worksheet.metadata.tienda).strip() == ""
        ):
            return True
    ...
    def _has_occupation_mapping_issues(self, worksheet):
        ...
        for cargo in worksheet.data["cargo"].dropna().unique():
            if cargo and str(cargo).strip():
                ...
                if any(
                    indicator in cargo_str
                    for indicator in ["(A)", "(B)", "(C)", "P/T", "PT"]
                ):
                    return True
...
class DataPreviewFrame:
    ...
    def _create_details_widgets(self):
        ...
        ttk.Label(selector_frame, text="Select Worksheet:").pack(
            side="left", padx=(0, 5)
        )
        self.worksheet_var = tk.StringVar()
        self.worksheet_combo = ttk.Combobox(
            selector_frame, textvariable=self.worksheet_var, state="readonly"
        )
        self.worksheet_combo.pack(side="left", padx=(0, 10))
        self.worksheet_combo.bind("<<ComboboxSelected>>", self._on_worksheet_selected)

        ttk.Button(
            selector_frame, text="Refresh", command=self._refresh_worksheet_details
        ).pack(side="left")
        ...
        self.metadata_label = ttk.Label(
            data_frame, text="", font=("TkDefaultFont", 8), foreground="gray"
        )
    ...
    def _create_logs_widgets(self):
        ...
        ttk.Label(
            control_frame, text="Application Logs:", font=("TkDefaultFont", 9)
        ).pack(side="left")

        # Log text area
        self.log_text = scrolledtext.ScrolledText(
            self.logs_frame, wrap=tk.WORD, font=("Consolas", 9), state="disabled"
        )
    ...
    def _create_uniform_widgets(self):
        ...
        self.uniform_worksheet_combo = ttk.Combobox(
            selector_frame, textvariable=self.uniform_worksheet_var, state="readonly"
        )
        self.uniform_worksheet_combo.pack(side="left", padx=(0, 10))
        self.uniform_worksheet_combo.bind(
            "<<ComboboxSelected>>", self._on_uniform_worksheet_selected
        )

        ttk.Button(
            selector_frame, text="Refresh", command=self._refresh_uniform_details
        ).pack(side="left")

        # Uniform data treeview
        uniform_data_frame = ttk.LabelFrame(
            self.uniform_frame, text="Uniforms by Person", padding=5
        )
        uniform_data_frame.pack(fill="both", expand=True, padx=5, pady=5)

        # Metadata footnote for uniform data
        self.uniform_metadata_label = ttk.Label(
            uniform_data_frame, text="", font=("TkDefaultFont", 8), foreground="gray"
        )
        ...
        uniform_scroll_y = ttk.Scrollbar(
            uniform_data_frame, orient="vertical", command=self.uniform_data_tree.yview
        )
        uniform_scroll_y.pack(side="right", fill="y")
        self.uniform_data_tree.configure(yscrollcommand=uniform_scroll_y.set)

        uniform_scroll_x = ttk.Scrollbar(
            uniform_data_frame,
            orient="horizontal",
            command=self.uniform_data_tree.xview,
        )
    ...
    def _setup_uniform_columns(self, worksheet):
        ...
        uniform_df = (
            worksheet.uniform_data if worksheet.uniform_data is not None else None
        )
        ...
        return (
            ["Name", "Cargo"]
            + [str(c) for c in talla_cols]
            + [str(c) for c in uniform_cols_order]
        )
    ...
    def _populate_uniform_data(self, worksheet, columns):
        ...
        for idx in range(max_rows):
            ...
            for ucol in uniform_cols_order:
                display = self._get_uniform_display_value(
                    uniform_row, ucol, cargo_upper, packer_cols, mozo_cols
                )
    ...
    def _extract_name_and_cargo(self, series):
        ...
        return str(name_val) if pd.notna(name_val) else "", str(cargo_val) if pd.notna(
            cargo_val
        ) else ""

    def _get_uniform_display_value(
        self, uniform_row, ucol, cargo_upper, packer_cols, mozo_cols
    ):
        ...
        if cargo_upper == "PACKER" and ucol in packer_cols:
            ...
            cargo_upper not in ("PACKER",)
            and ucol not in packer_cols
            and ucol not in mozo_cols
        ):
            should_display = True
    ...
    def _update_data_tree(self, worksheet):
        ...
        self._populate_data_tree(
            worksheet.data, columns, columns_with_occupation, fecha_cols
        )
    ...
    def _identify_fecha_columns(self, columns):
        ...
        for col in columns:
            col_l = str(col).lower()
            if ("fecha" in col_l and "ingreso" in col_l) or col_l.strip() in (
                "fecha de ingreso",
                "fecha ingreso",
            ):
                fecha_cols.add(col)
    ...
    def _populate_data_tree(
        self, data, original_columns, columns_with_occupation, fecha_cols
    ):
        """Populate the data treeview with actual data."""
        for index, row in data.head(
            100
        ).iterrows():  # Limit to 100 rows for performance
            row_values = []
            ...
            if cargo and str(cargo).strip():
                ...
                normalized_occupation = self.unified_service.normalize_occupation(
                    str(cargo)
                )
    ...
    def add_log_message(self, message: str, level: str = "INFO"):
        ...
        if level == "ERROR":
            self.notebook.select(
                3
            )  # Logs tab (now 4th tab: Summary, Details, Uniforms, Logs)
...
class CargosTab:
    ...
    def __init__(
        self, parent, config: AppConfig, unified_service: UnifiedConfigService
    ):
        self.frame = ttk.Frame(parent)
        self.config = config
        self.unified_service = unified_service
    ...
    def _setup_callbacks(self):
        ...
        self.file_selection.on_autorizacion_template_changed = (
            self._on_autorizacion_template_changed
        )
        self.file_selection.on_destination_path_changed = (
        ...
        self.file_selection.on_template_toggles_changed = (
            self._on_template_toggles_changed
        )
    ...
    def _on_template_toggles_changed(self):
        ...
        self.log_message(
            f"Template selection changed: {', '.join(enabled_templates) if enabled_templates else 'None selected'}"
        )
    ...
    def _on_generate_files(self):
        ...
        try:
            locales = []
            if (
                self.data_preview.current_excel_data
                and self.data_preview.current_excel_data.worksheets
            ):
    ...
    def _update_generate_enablement(self):
        ...
        try:
            has_data = (
                hasattr(self, "data_preview")
                and self.data_preview.current_excel_data
                and self.data_preview.current_excel_data.successful_worksheets > 0
            )
            ...
            enabled_templates = self.file_selection.get_enabled_templates()
            templates_ok = (
                len(enabled_templates) > 0
            )  # At least one template must be enabled

            if templates_ok and "AUTORIZACION" in enabled_templates:
                autorizacion_errors = TemplateValidator.validate_autorizacion_template(
                    self.config
                )
            ...
            self.generate_button.set_enabled(
                has_data and has_locales_available and templates_ok and dest_ok
            )
        except Exception:
    ...
    def _open_generation_options_dialog(self, locales: List[str]):
        ...
        ttk.Label(
            left_frame, text="Select Locales:", font=("TkDefaultFont", 10, "bold")
        ...
        scrollbar = ttk.Scrollbar(
            checkbox_frame_container, orient="vertical", command=canvas.yview
        )
        ...
        ttk.Button(btn_frame, text="Select All", command=_select_all).pack(
            side="left", padx=(0, 5)
        )
        ttk.Button(btn_frame, text="Deselect All", command=_deselect_all).pack(
            side="left"
        )
        ...
        ttk.Checkbutton(
            left_frame,
            text="Also generate combined DOCX per local",
        ...
        preview_text = tk.Text(
            right_frame, wrap=tk.WORD, height=20, width=40, font=("Consolas", 9)
        )
        preview_scroll = ttk.Scrollbar(
            right_frame, orient="vertical", command=preview_text.yview
        )
                ...
                if people_count > 0:
                    ...
                    for template in enabled_templates:
                        for i in range(min(people_count, 3)):  # Show first 3
                            preview_text.insert(
                                tk.END, f"   📄 {template}_person_{i + 1}.docx\n"
                            )
                        if people_count > 3:
                            preview_text.insert(
                                tk.END,
                                f"   ... and {people_count - 3} more {template} documents\n",
                            )
                    ...
                    if combine_var.get():
                        for template in enabled_templates:
                            preview_text.insert(
                                tk.END, f"   📄 {template}_COMBINED_{locale}.docx\n"
                            )
                            total_docs += 1
                else:
        ...
        ttk.Button(actions, text="Generate", command=_ok).pack(
            side="right", padx=(6, 8)
        )
...
class ConfigurationTab:
    ...
    def _create_widgets(self):
        ...
        ttk.Button(
            button_frame, text="Add Occupation", command=self._add_occupation
        ).pack(side="left", padx=(0, 10))
        ttk.Button(button_frame, text="Add Prenda", command=self._add_prenda).pack(
            side="left", padx=(0, 10)
        )
        ttk.Button(
            button_frame,
            text="📁 Reload Prices from Excel",
            command=self._reload_prices_from_excel,
        ).pack(side="left")
    ...
    def _create_occupations_tab(self):
        ...
        self.occupations_tree = ttk.Treeview(
            occupations_frame, columns=columns, show="headings", height=15
        )
        ...
        occupations_scrollbar = ttk.Scrollbar(
            occupations_frame, orient="vertical", command=self.occupations_tree.yview
        )
    ...
    def _create_pricing_matrix_tab(self):
        ...
        self.pricing_tree = ttk.Treeview(
            pricing_frame, columns=columns, show="headings", height=15
        )
        ...
        pricing_scrollbar = ttk.Scrollbar(
            pricing_frame, orient="vertical", command=self.pricing_tree.yview
        )
    ...
    def _load_data(self):
        ...
        try:
            ...
            self.pricing_tree.insert(
                "", "end", values=("Error", "Loading", "Failed", "", "")
            )
    ...
    def _edit_price_entry(self, event):
        ...
        # Create edit dialog
        self._show_price_edit_dialog(
            occupation_name, prenda_type, size_group, local_group, price
        )
    ...
    def _show_occupation_edit_dialog(self, occupation):
        ...
        ttk.Label(dlg, text="Display Name:").grid(
            row=1, column=0, sticky="w", padx=5, pady=5
        )
        display_var = tk.StringVar(value=occupation.display_name if occupation else "")
        ttk.Entry(dlg, textvariable=display_var, width=40).grid(
            row=1, column=1, padx=5, pady=5
        )

        ttk.Label(dlg, text="Synonyms (one per line):").grid(
        ...
        ttk.Label(dlg, text="Description:").grid(
        ...
        def save_occupation():
            ...
            synonyms_list = [
                s.strip() for s in synonyms_text_content.split("\n") if s.strip()
            ]
                ...
                if occupation:
                    if self.unified_service.update_occupation(new_occupation):
                        ...
                        if not self.unified_service.save():
                            messagebox.showwarning(
                                "Warning",
                                "Occupation updated but failed to save to file!",
                            )
                        self._load_data()
                        dlg.destroy()
                    else:
                    ...
                    if self.unified_service.add_occupation(new_occupation):
                        ...
                        if not self.unified_service.save():
                            messagebox.showwarning(
                                "Warning",
                                "Occupation added but failed to save to file!",
                            )
                        self._load_data()
                        dlg.destroy()
                    else:
        ...
        ttk.Button(button_frame, text="Save", command=save_occupation).pack(
            side="left", padx=5
        )
        ttk.Button(button_frame, text="Cancel", command=dlg.destroy).pack(
            side="left", padx=5
        )
    ...
    def _show_prenda_edit_dialog(self, occupation_name, prenda):
        ...
        # Form fields
        ttk.Label(dlg, text="Occupation:").grid(
        ...
        occ_combo = ttk.Combobox(
            ...
            values=[
                occ.name for occ in self.unified_service.unified_config.occupations
            ],
            width=37,
        )
        occ_combo.grid(row=0, column=1, padx=5, pady=5)

        ttk.Label(dlg, text="Prenda Type:").grid(
            row=1, column=0, sticky="w", padx=5, pady=5
        )
        prenda_var = tk.StringVar(value=prenda.prenda_type if prenda else "")
        ttk.Entry(dlg, textvariable=prenda_var, width=40).grid(
            row=1, column=1, padx=5, pady=5
        )
        ...
        ttk.Entry(dlg, textvariable=display_name_var, width=40).grid(
            row=2, column=1, padx=5, pady=5
        )

        has_sizes_var = tk.BooleanVar(value=prenda.has_sizes if prenda else True)
        ttk.Checkbutton(dlg, text="Has Sizes", variable=has_sizes_var).grid(
            row=3, column=1, sticky="w", padx=5, pady=5
        )

        is_required_var = tk.BooleanVar(value=prenda.is_required if prenda else False)
        ttk.Checkbutton(dlg, text="Required", variable=is_required_var).grid(
            row=4, column=1, sticky="w", padx=5, pady=5
        )

        ttk.Label(dlg, text="Default Quantity:").grid(
            row=5, column=0, sticky="w", padx=5, pady=5
        )
        qty_var = tk.StringVar(value=str(prenda.default_quantity) if prenda else "0")
        ttk.Entry(dlg, textvariable=qty_var, width=40).grid(
            row=5, column=1, padx=5, pady=5
        )

        # Pricing section
        ttk.Label(dlg, text="Pricing (S/):", font=("TkDefaultFont", 10, "bold")).grid(
        ...
        for i, size in enumerate(["SML", "XL", "XXL"], 1):
            ...
            for j, local in enumerate(["other", "tarapoto", "san_isidro"], 1):
                ...
                ttk.Entry(
                    pricing_frame, textvariable=price_vars[price_attr], width=8
        ...
        def save_prenda():
            ...
            for price_attr, var in price_vars.items():
                try:
                    ...
                    messagebox.showerror(
                        "Error", f"Please enter a valid price for {price_attr}!"
                    )
                    return
            ...
            try:
                if self.unified_service.add_prenda_to_occupation(
                    occ_var.get(), new_prenda
                ):
                    self._load_data()
                    dlg.destroy()
                else:
        ...
        ttk.Button(button_frame, text="Save", command=save_prenda).pack(
            side="left", padx=5
        )
        ttk.Button(button_frame, text="Cancel", command=dlg.destroy).pack(
            side="left", padx=5
    ...
    def _show_price_edit_dialog(
        self, occupation_name, prenda_type, size_group, local_group, price
    ):
        ...
        ttk.Entry(dlg, textvariable=occ_var, state="readonly", width=30).grid(
            row=0, column=1, padx=5, pady=5
        )
        ...
        ttk.Entry(dlg, textvariable=prenda_var, state="readonly", width=30).grid(
            row=1, column=1, padx=5, pady=5
        )

        ttk.Label(dlg, text="Size Group:").grid(
            row=2, column=0, sticky="w", padx=5, pady=5
        )
        size_var = tk.StringVar(value=size_group)
        ttk.Entry(dlg, textvariable=size_var, state="readonly", width=30).grid(
            row=2, column=1, padx=5, pady=5
        )

        ttk.Label(dlg, text="Local Group:").grid(
            row=3, column=0, sticky="w", padx=5, pady=5
        )
        local_var = tk.StringVar(value=local_group)
        ttk.Entry(dlg, textvariable=local_var, state="readonly", width=30).grid(
            row=3, column=1, padx=5, pady=5
        )

        ttk.Label(dlg, text="Price:").grid(row=4, column=0, sticky="w", padx=5, pady=5)
        price_var = tk.StringVar(value=str(price))
        ttk.Entry(dlg, textvariable=price_var, width=30).grid(
        ...
        ttk.Button(button_frame, text="Save", command=save_price).pack(
            side="left", padx=5
        )
        ttk.Button(button_frame, text="Cancel", command=dlg.destroy).pack(
            side="left", padx=5
...
def show_gender_selection_dialog(
    root: tk.Tk,
    person_name: str,
    cargo: str,
    male_option: str,
    female_option: str,
    male_prices: dict = None,
    female_prices: dict = None,
) -> str:
    ...
    ttk.Label(
        info_frame, text=f"Empleado: {person_name}", font=("Helvetica", 14, "bold")
    ).pack(anchor="center")
    ttk.Label(info_frame, text=f"Cargo: {cargo}", font=("Helvetica", 12)).pack(
        anchor="center", pady=(5, 0)
    )
    ttk.Label(
        info_frame, text="Seleccione el tipo de uniforme:", font=("Helvetica", 11)
    ...
    ttk.Label(male_frame, text=male_option, font=("Helvetica", 11, "bold")).pack(
        anchor="w", pady=(0, 10)
    )

    if male_prices:
        ttk.Label(
            male_frame, text="Precios Referenciales:", font=("Helvetica", 10, "italic")
        ).pack(anchor="w", pady=(0, 5))
        ...
        for i, (prenda, price) in enumerate(sorted(male_prices.items())):
            ...
            ttk.Label(price_container, text=price_text, font=("Helvetica", 10)).pack(
                anchor="w", pady=2
            )
    else:
        ttk.Label(
            male_frame,
            text="(Precios no disponibles)",
            foreground="gray",
    ...
    ttk.Label(female_frame, text=female_option, font=("Helvetica", 11, "bold")).pack(
        anchor="w", pady=(0, 10)
    )
    ...
    if female_prices:
        ...
        for i, (prenda, price) in enumerate(sorted(female_prices.items())):
            price_text = f"• {prenda}: S/{price:.2f}"
            ttk.Label(price_container_f, text=price_text, font=("Helvetica", 10)).pack(
                anchor="w", pady=2
            )
    else:
        ttk.Label(
    ...
    ttk.Button(
        button_frame,
        text="Seleccionar HOMBRE",
        command=select_male,
        style="Large.TButton",
    ).pack(side="left", fill="x", expand=True, padx=(0, 10), ipady=5)
    ttk.Button(
        button_frame,
        text="Seleccionar MUJER",
        command=select_female,
        style="Large.TButton",
    ).pack(side="right", fill="x", expand=True, padx=(10, 0), ipady=5)
